...
class PaperEntryOrderTracker:
    ...
    def _load(self) -> list[PaperEntryOrderRecord]:
        ...
        try:
            ...
            records = [PaperEntryOrderRecord(**item) for item in data["orders"]]
    ...
    def _save(self) -> None:
        ...
        temporary.write_text(
            json.dumps({"version": 2, "orders": [asdict(item) for item in self.records]}, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
    ...
    def register_submitted(self, order: Any, underlying_ticker: str) -> PaperEntryOrderRecord:
        ...
        record = PaperEntryOrderRecord(
            True, trading_date.isoformat(), _required_text(_value(order, "id"), "Broker order ID"),
